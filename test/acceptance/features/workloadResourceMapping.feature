Feature: Bind services to workloads based on workload resource mapping
    ...
    Scenario: Mapping should allow bindAsFiles: false in non-podspec resources
        ...
        When Service Binding is applied
            ...
            """
        Then Service Binding is ready
        And Secret has been injected in to CR "$scenario_id-npc" of kind "notpodspecs.stable.example.com" at path "{.spec.initContainerSpecs[0].envFrom[0].secretRef.name}"
        And Secret has been injected in to CR "$scenario_id-npc" of kind "notpodspecs.stable.example.com" at path "{.spec.containerSpecs[0].envFrom[0].secretRef.name}"
    ...
    @spec
    Scenario: Projecting environment variables should use the specified env field
        Given The Secret is present
            """
            apiVersion: v1
            kind: Secret
        ...
        When Service Binding is applied
            ...
            """
        Then Service Binding is ready
        And The application env var "BINDING_TYPE" has value "secret"
        And The application env var "BINDING_USERNAME" has value "AzureDiamond"
        And The application env var "BINDING_PASSWORD" has value "hunter2"
